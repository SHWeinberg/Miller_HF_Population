...
for iBeat = size(time, 1)-4:size(time, 1)
    ...
    Xhere = X{iBeat};
     ...
     %extraParams.HF_flag = parameters.HF_flag;
...
if ~isempty(time)
    ...
    [~,Ui] = unique(currents.time); %unique index

    % Pass out only the unique solutions:
    currents.Cai = currents.Cai(Ui);
    currents.Cass = currents.Cass(Ui);
    currents.Jrel=currents.Jrel(Ui);
    currents.INa=currents.INa(Ui);
    currents.INaL=currents.INaL(Ui);
    currents.Ito=currents.Ito(Ui);
    currents.ICaL=currents.ICaL(Ui);
    currents.IKr=currents.IKr(Ui);
    currents.IKs=currents.IKs(Ui);
    currents.IK1=currents.IK1(Ui);
    currents.INaCa_i=currents.INaCa_i(Ui);
    currents.INaCa_ss=currents.INaCa_ss(Ui);
    currents.INaK=currents.INaK(Ui);
    currents.IKb=currents.IKb(Ui);
    currents.INab=currents.INab(Ui);
    currents.ICab=currents.ICab(Ui);
    currents.IpCa=currents.IpCa(Ui);
    currents.Jdiff=currents.Jdiff(Ui);
    currents.JdiffNa=currents.JdiffNa(Ui);
    currents.JdiffK=currents.JdiffK(Ui);
    currents.Jup=currents.Jup(Ui);
    currents.Jleak=currents.Jleak(Ui);
    currents.Jtr=currents.Jtr(Ui);
    currents.CaMKa=currents.CaMKa(Ui);
    currents.Istim=currents.Istim(Ui);
    currents.fINap = currents.fINap(Ui);
    currents.fINaLp = currents.fINaLp(Ui);
    currents.fICaLp =currents.fICaLp(Ui);
    currents.fJrelp =currents.fJrelp(Ui);
    currents.fJupp =currents.fJupp(Ui);
    currents.CaJSR = currents.CaJSR(Ui);
    currents.CaNSR =currents.CaNSR(Ui);
    currents.PhiCaL_ss =currents.PhiCaL_ss(Ui);
    currents.V = currents.V(Ui); %  This may be usually obtained directly from vector of state variables, but not necessarily when clamping is used, so it can be also extracted separately.
    currents.ICaL_i =currents.ICaL_i(Ui);
    currents.IClCa = currents.IClCa(Ui);
    currents.IClbk = currents.IClbk(Ui);
    currents.ICaL_tot = currents.ICaL_tot(Ui);
    currents.time = currents.time(Ui);
    currents.Irel = currents.Irel(Ui);
    currents.ICaL_ss = currents.ICaL_ss(Ui);
    currents.JSRstart = currents.JSRstart(Ui);
    currents.JSRend = currents.JSRend(Ui);
    currents.caLeavingNCX = currents.caLeavingNCX(Ui);
    currents.caLeavingJup = currents.caLeavingJup(Ui);
    currents.caLeavingPCa = currents.caLeavingPCa(Ui);
    currents.INaCa = currents.INaCa(Ui);

    currents.fICaLP = currents.fICaLP(Ui);
    currents.fIKsP = currents.fIKsP(Ui);
    currents.fPLBP = currents.fPLBP(Ui);
    currents.fTnIP = currents.fTnIP(Ui);
    currents.fINaP = currents.fINaP(Ui);
    currents.fINaKP = currents.fINaKP(Ui);
    currents.fRyRP = currents.fRyRP(Ui);
    currents.fIKurP = currents.fIKurP(Ui);
    %currents.Ta = currents.Ta;

    currents.nai = currents.nai(Ui);
    currents.ki = currents.ki(Ui);
end
