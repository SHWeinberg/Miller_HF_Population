...
function output=model_Torord_dynCl_BARS_EP_Uncoupled_HF(t,X,flag_ode, cellType, ICaL_Multiplier, ...
    ...
    INaK_Multiplier, INab_Multiplier, ICab_Multiplier, IpCa_Multiplier, IClCa_Multiplier, IClb_Multiplier, Jrel_Multiplier,Jup_Multiplier,CaMKa_Multiplier, Jleak_Multiplier, nao,cao,ko,ICaL_fractionSS,INaCa_fractionSS, stimAmp, stimDur, vcParameters, apClamp, extraParams)
...
%CaMK constants
KmCaMK=0.15; % half saturation of CaMK

aCaMK=0.05*CaMKa_Multiplier; %Rate of phosphorylation of CaMKII
bCaMK=0.00068; % rate of dephosphorylation by CaMKII
CaMKo=0.05; % equilibrium fraction of active CaMKII binding sites
KmCaM=0.0015;  %half-saturation coefficient of substrate parameters

%update CaMK
CaMKb=CaMKo*(1.0-CaMKt)/(1.0+KmCaM/cass); %fraction of CaMKII binding sites bound to Ca2+/Cal
CaMKa=CaMKb+CaMKt; % fraction of active binding sites
...
if settings.runSignalingPathway == 1
    ...
    pp1_PP1f_cyt_sum = c(38) - c(37) + X(59+39); % eq. to affinity for PP1 -
    % Concentration of uninhibited PP1 in the cytosolic compartment
    ...
    % replace assumed PP1 value with the calculated value from the
    % signaling steady state for given signaling value. 
    % Concentration of uninhibited PP1 in the cytosolic compartment
    pp1_PP1f_cyt_sum = c(38) - c(37) + X(59+39); % eq. to affinity for PP1 -
    % Concentration of uninhibited PP1 in the cytosolic compartment
    PP1f_cyt = 0.5 * (sqrt(pp1_PP1f_cyt_sum ^ 2.0 + 4.0 * c(38) * c(37)) - pp1_PP1f_cyt_sum);
...
[Jup, Jleak] = getJup_ORd2011(cai, cansr, fJupp, celltype, Jup_Multiplier,Jleak_Multiplier,fPLBP);
...
%% Jup
function [Jup, Jleak] = getJup_ORd2011(cai, cansr, fJupp, celltype, Jup_Multiplier,Jleak_Multiplier,fPLBP)
...
Jleak=Jleak_Multiplier*Jup_Multiplier* 0.0048825*cansr/15.0;
